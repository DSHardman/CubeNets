...
#define NUM_MEAS           NUM_ELECTRODES*(NUM_ELECTRODES-1)*(NUM_ELECTRODES-2)*(NUM_ELECTRODES-3)
...
uint32_t read_signal_at_freq(int freq, double * rms, double * mag, double * phase, uint16_t * error_rate, uint8_t debug)
{ 
    uint16_t i, j;
    uint16_t phase_count;
    uint16_t adc_min = 1023;
...
void calibrate_samples_at_freq(int freq) {
    ...
    /* Take 10000 samples to determine sample rate */
    num_samples = 10000;
    uint32_t sample_time = read_signal_at_freq(freq, NULL, NULL, NULL, NULL, 0);
...
void read_all_at_freq(int freq, double * rms_array, double * mag_array, double * phase_array, uint8_t num_elec) // added by david 11/12/23
{
    ...
    for(int src_pin = 0; src_pin < 6; src_pin++)
    {
        for(int sink_pin = 0; sink_pin < 6; sink_pin++)
        {
            if(sink_pin != src_pin)
            {
                mux_write(CHIP_SEL_MUX_SRC, elec_to_mux[src_pin], MUX_EN);
                ...
                for(vp_pin = 0; vp_pin < 6; vp_pin++)
                {
                    if((vp_pin != src_pin)&&(vp_pin != sink_pin))
                    {
                        for(vn_pin = 0; vn_pin < 6; vn_pin++)
                        {
