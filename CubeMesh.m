...
for i = 1:length(fmdl.elems)
    corners = [fmdl.nodes(fmdl.elems(i, 1), :);...
        fmdl.nodes(fmdl.elems(i, 2), :);...
        fmdl.nodes(fmdl.elems(i, 3), :)];
    centroid = mean(corners);
    if rssq(centroid-location1) <= radius || rssq(centroid-location2) <= radius
        sim_img.elem_data(i) = inclusion_c;
    end
end

sim_img.fwd_solve.get_all_meas = 1;
vc = fwd_solve(sim_img);
...
maxval = 0; channel = 0;
for i = 1:size(vh.volt, 2)
    if mean(abs(vc.volt(:, i) - vh.volt(:, i))) > maxval
        maxval = mean(abs(vc.volt(:, i) - vh.volt(:, i)));
        channel = i;
    end
end
...
rec_img= inv_solve(inv2d, vh.meas, vc.meas);
